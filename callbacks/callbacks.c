...
void wait_exit() {
  ...
  if (close_future) {
      fprintf(stderr,"@@@@ cass_future_wait(close_future) 0x%08x\n", close_future);
    cass_future_wait(close_future);
      fprintf(stderr,"@@@@ cass_future_free(close_future) 0x%08x\n", close_future);
...
void signal_exit(CassSession* session) {
  ...
  if (session) {
    close_future = cass_session_close(session);
      fprintf(stderr,"@@@@ cass_session_close(session) 0x%08x\n", close_future);
  }
  exit_flag = 1;
  uv_cond_signal(&cond);
  uv_mutex_unlock(&mutex);
...
void print_error(CassFuture* future) {
  ...
  cass_future_error_message(future, &message, &message_length);
    fprintf(stderr,"@@@@ cass_future_error_message(future, &message, &message_length) 0x%08x\n", future);
...
void connect_session(CassSession* session, const CassCluster* cluster, CassFutureCallback callback) {
  CassFuture* future = cass_session_connect(session, cluster);
    fprintf(stderr,"@@@@ cass_session_connect(session, cluster) 0x%08x\n", future);
  cass_future_set_callback(future, callback, session);
    fprintf(stderr,"@@@@ cass_future_set_callback(future, callback, session) 0x%08x\n", future);
  cass_future_free(future);
    fprintf(stderr,"@@@@ cass_future_free(future) 0x%08x\n", future);
}

void execute_query(CassSession* session, const char* query,
                   CassFutureCallback callback) {
  CassStatement* statement = cass_statement_new(query, 0);
    fprintf(stderr,"@@@@ cass_statement_new(query, 0) 0x%08x\n", statement);
  CassFuture* future = cass_session_execute(session, statement);
    fprintf(stderr,"@@@@ cass_session_execute(session, statement) 0x%08x\n", future);
  cass_future_set_callback(future, callback, session);
    fprintf(stderr,"@@@@ cass_future_set_callback(future, callback, session) 0x%08x\n", future);
  cass_future_free(future);
    fprintf(stderr,"@@@@ cass_future_free(future) 0x%08x\n", future);
  cass_statement_free(statement);
    fprintf(stderr,"@@@@ cass_statement_free(statement) 0x%08x\n", statement);
}

void on_session_connect(CassFuture* future, void* data) {
  CassSession* session = (CassSession*)data;
  CassError code = cass_future_error_code(future);
    fprintf(stderr,"@@@@ cass_future_error_code(future) 0x%08x\n", future);
  if (code != CASS_OK) {
      fprintf(stderr,"on_session_connect: code=%d\n",code);
    print_error(future);
    signal_exit(NULL);
    return;
  }
...
void on_create_table(CassFuture* future, void* data) {
  ...
  statement = cass_statement_new(insert_query, 2);
    fprintf(stderr,"@@@@ cass_statement_new(insert_query, 2) 0x%08x\n", statement);
  ...
  insert_future = cass_session_execute((CassSession*)data, statement);
    fprintf(stderr,"@@@@ cass_session_execute((CassSession*)data, statement) 0x%08x\n", insert_future);

  cass_future_set_callback(insert_future, on_insert, data);
    fprintf(stderr,"@@@@ cass_future_set_callback(insert_future, on_insert, data) 0x%08x\n", insert_future);

  cass_statement_free(statement);
    fprintf(stderr,"@@@@ cass_statement_free(statement) 0x%08x\n", statement);
  cass_future_free(insert_future);
    fprintf(stderr,"@@@@ cass_future_free(insert_future) 0x%08x\n", insert_future);
}
...
void on_insert(CassFuture* future, void* data) {
  ...
  if (code != CASS_OK) {
    ...
    CassStatement* statement
      ...
      fprintf(stderr,"@@@@ cass_statement_new(select_query, 0) 0x%08x\n", statement);
    ...
    cass_future_set_callback(select_future, on_select, data);
      fprintf(stderr,"@@@@ cass_future_set_callback(select_future, on_select, data) 0x%08x\n", select_future);

    cass_statement_free(statement);
      fprintf(stderr,"@@@@ cass_statement_free(statement) 0x%08x\n", statement);
    cass_future_free(select_future);
      fprintf(stderr,"@@@@ cass_future_free(select_future) 0x%08x\n", select_future);
  }
}
...
void on_select(CassFuture* future, void* data) {
  ...
  if (code != CASS_OK) {
    ...
    const CassResult* result = cass_future_get_result(future);
      fprintf(stderr,"@@@@ cass_future_get_result(future) 0x%08x\n", result);
    ...
    cass_result_free(result);
      fprintf(stderr,"@@@@ cass_result_free(result) 0x%08x\n", result);
  }
...
int main(int argc, char* argv[]) {
  ...
  CassSession* session = cass_session_new();
    fprintf(stderr,"@@@@ cass_session_new() 0x%08x\n", session);
  ...
  cass_session_free(session);
    fprintf(stderr,"@@@@ cass_session_free(session) 0x%08x\n", session);
